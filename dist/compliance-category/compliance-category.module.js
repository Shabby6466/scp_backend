"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ComplianceCategoryModule = void 0;
const common_1 = require("@nestjs/common");
const compliance_category_controller_js_1 = require("./compliance-category.controller.js");
const compliance_category_service_js_1 = require("./compliance-category.service.js");
let ComplianceCategoryModule = class ComplianceCategoryModule {
};
exports.ComplianceCategoryModule = ComplianceCategoryModule;
exports.ComplianceCategoryModule = ComplianceCategoryModule = __decorate([
    (0, common_1.Module)({
        controllers: [compliance_category_controller_js_1.ComplianceCategoryController],
        providers: [compliance_category_service_js_1.ComplianceCategoryService],
        exports: [compliance_category_service_js_1.ComplianceCategoryService],
    })
], ComplianceCategoryModule);
//# sourceMappingURL=compliance-category.module.js.map